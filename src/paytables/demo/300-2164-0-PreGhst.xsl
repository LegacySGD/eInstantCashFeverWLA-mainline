			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						const SYMB_COLLECT_BONUS = 'T';
						const SYMB_WHEEL_BONUS = 'U';
						const SYMB_IW_SINGLE = 'V';
						const SYMB_IW_DOUBLE = 'W';
						...
						r.push('<td colspan="' + winningNums.length + '">' + getTranslationByName("luckyNumbers", translations) + '</td>');
 						r.push('</tr>');
						
 						r.push('<tr>');

 						for (var i = 0; i < winningNums.length; i++) {
 							r.push('<td class="tablebody">' + winningNums[i] + '</td>');
 						}
						...
						r.push('<tr class="tablehead">');
 						...
 						r.push('<td>' + getTranslationByName("cashValues", translations) + '</td>');
 						r.push('</tr>');
						...
						for (var i = 0; i < outcomeNums.length; i++) {
							...
							if (outcomeNums[i] == SYMB_IW_DOUBLE) {
 								...
 								if (checkMatch(winningNums, outcomeNums[i]) || outcomeNums[i] == SYMB_IW_SINGLE || outcomeNums[i] == SYMB_IW_DOUBLE) {
						...
						if (booleanBonusGame) {
							...
							r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
							r.push('<tr class="tablehead">');
							r.push('<td>' + getTranslationByName("prizeBonusWins", translations) + '</td>');
							r.push('</tr>');

							for (var i = 0; i < outcomeBonusGame.length-1; i++) {
								r.push('<tr class="tablebody">');
								r.push('<td>' + convertedPrizeValues[getPrizeNameIndex(prizeNames, outcomeBonusGame[i])] + '</td>');
 								r.push('</tr>');
							}
						...
						if (booleanWheelGame) {
							...
							r.push('<table border="0" cellpadding="2" cellspacing="1"class="gameDetailsTable">');
							r.push('<tr class="tablehead">');
							r.push('<td>' + getTranslationByName("wheelBonusWins", translations) + '</td>');
							r.push('</tr>');
					
							r.push('<tr class="tablebody">');
							r.push('<td>' + convertedPrizeValues[getPrizeNameIndex(prizeNames, outcomeWheelGame[0])] + ' x ' + outcomeWheelGame[1] + '</td>');
 							r.push('</tr>');
							r.push('</table>');
						}
